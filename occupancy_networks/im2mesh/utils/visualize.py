...
def visualize_data(data, data_type, out_file):
    ...
    if data_type == 'img':
        # ipdb.set_trace()
        # if out_file.split('.')[-1] == 'tif' and data.dim() == 3:
        #     data = np.asarray(data).astype(np.uint16)
        #     imwrite( out_file, data, imagej=True, resolution=(1./.109,1./.109), 
        #     metadata= {'spacing':0.250, 'unit':'um','axes':'ZYX' })
        # else:
        if data.dim() == 3:
