...
parser.add_argument('--emitters_folder', type=str,
                    help='Output path for points.')
parser.add_argument('--emitter_spacing', type=float, default=389.71143,
                    help='One emitter in area of ... nm^2.')
parser.add_argument('--resolution', type=int, default=24,
                    help='Resolution of the EM dataset.')
...
def process_path(in_path, args):
    ...
    if args.emitters_folder is not None:
        export_emitters(mesh, modelname, loc, scale, args)
...
def export_emitters(mesh, modelname, loc, scale, args):
    # if not mesh.is_watertight:
    #     print('Warning: mesh %s is not watertight!'
    #           'Cannot sample emitters.' % modelname)
    #     return

    filename = os.path.join(args.emitters_folder, modelname + '.npz')
    if not args.overwrite and os.path.exists(filename):
        print('Emitter points already exist: %s' % filename)
        return

    mesh_area = mesh.area
    num_emitters = int(np.round(
                    (args.resolution*args.resolution*mesh_area)/args.emitter_spacing)) *2000 
    density = 4
    # num_emitters_legacy= int((mesh_area/1000)*density)
    points_surface = mesh.sample(num_emitters)
    # print(num_emitters, num_emitters_legacy)
    # return
    ipdb.set_trace()
    if len(points_surface) == 0 :
        print('WARN: No emitters for', filename)
        return
    # Compress
    if args.float16:
        dtype = np.float16
    ...
    print('Writing ',num_emitters, ' emitters: %s' % filename)
    np.savez(filename, points=points_surface, 
             loc=loc, scale=scale)
