...
min_wavelength = 0.595 #0.436 # microns
...
def gLXYZParticleScan(mp, dxy, xy_size, pz, normalize = True, wvl = 0.6, zd = None, zv = 0.0, px=0, py=0):
    """
...
def psfRZToPSFXYZ(dxy, xy_size, zv, zv_, rv, PSF_rz, px=0, py=0):
    ...
    R_pixel = numpy.sqrt((X - c_xy - px[:,None,None]) * (X - c_xy - px[:,None,None]) + (Y - c_xy - py[:,None,None]) * (Y - c_xy - py[:,None,None])).ravel()
    R_pixel *= 1/(rv[1] - rv[0])
